...
class ReleaseV060Tests(unittest.TestCase):
    ...
    def test_runtime_and_release_versions_align(self) -> None:
        self.assertEqual(elman_os.__version__, "0.6.0")
        self.assertEqual(DISPLAY_VERSION, "0.6.0")
        self.assertEqual(PACKAGE_VERSION, "0.6.0")

    def test_package_metadata_declares_v060(self) -> None:
        pyproject = (self.root / "pyproject.toml").read_text(encoding="utf-8")
        self.assertRegex(pyproject, r'(?m)^version = "0\.6\.0"$')

    def test_manifest_declares_validated_scope(self) -> None:
        self.assertEqual(self.manifest["version"], "0.6.0")
        self.assertEqual(
            self.manifest["verification_scope"]["kernel_unittests"], 278
        )

    def test_manifest_declares_stable_promotion(self) -> None:
        ...
        self.assertTrue(
            {
                ...
                "v060rc2_to_v060_migration_validation",
                "stable_v060_promotion",
                "deterministic_v060_archive",
            }.issubset(capabilities)
        )

    def test_stable_gates_are_explicit(self) -> None:
        scope = self.manifest["verification_scope"]
        self.assertTrue(self.manifest["release_candidate_validated"])
        self.assertTrue(self.manifest["final_release_approved"])
    ...
    def test_migration_guide_is_present(self) -> None:
        migration = self.root / "MIGRATION-v0.6.0-rc.2-to-v0.6.0.md"
    ...
    def test_changelog_starts_with_v060(self) -> None:
        changelog = (self.root / "CHANGELOG.md").read_text(encoding="utf-8")
        releases = re.findall(r"(?m)^## (v[^\n ]+)", changelog)
        self.assertTrue(releases)
        self.assertEqual(releases[0], "v0.6.0")

    def test_readme_identifies_v060(self) -> None:
        first_line = (
            (self.root / "README.md").read_text(encoding="utf-8").splitlines()[0]
        )
        self.assertEqual(first_line, "# ELMAN-OS Foundation Kit v0.6.0")

    def test_archive_builder_targets_v060(self) -> None:
        builder = (self.root / "scripts/build_release.py").read_text(
            encoding="utf-8"
        )
        self.assertIn(
            'ARCHIVE_PREFIX = "elman-os-foundation-kit-v0.6.0"', builder
        )
