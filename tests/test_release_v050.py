...
class ReleaseV050Tests(unittest.TestCase):
    ...
    def test_runtime_and_release_versions_align(self) -> None:
        self.assertEqual(elman_os.__version__, "0.5.0")
        self.assertEqual(DISPLAY_VERSION, "0.5.0")
        self.assertEqual(PACKAGE_VERSION, "0.5.0")

    def test_package_metadata_declares_v050(self) -> None:
    ...
    def test_migration_guide_is_present(self) -> None:
        migration = self.root / "MIGRATION-v0.4.0-to-v0.5.0.md"
        self.assertTrue(migration.is_file())
        self.assertIn("Retour arrière", migration.read_text(encoding="utf-8"))

    def test_changelog_starts_with_v050(self) -> None:
        changelog = (self.root / "CHANGELOG.md").read_text(encoding="utf-8")
        releases = re.findall(r"(?m)^## (v[^\n ]+)", changelog)
        self.assertTrue(releases)
        self.assertEqual(releases[0], "v0.5.0")

    def test_readme_identifies_v050(self) -> None:
        first_line = (
            (self.root / "README.md")
            .read_text(encoding="utf-8")
            .splitlines()[0]
        )
        self.assertEqual(first_line, "# ELMAN-OS Foundation Kit v0.5.0")

    def test_archive_builder_targets_v050(self) -> None:
        builder = (self.root / "scripts/build_release.py").read_text(
            encoding="utf-8"
        )
        self.assertIn(
            'ARCHIVE_PREFIX = "elman-os-foundation-kit-v0.5.0"',
            builder,
        )
